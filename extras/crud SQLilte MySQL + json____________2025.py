...
def grabar_pkl (archivo="instalador_2025",**diccionario_entrada):
    with open(f"{archivo}.pkl", mode="wb") as obj_pkl:
        pickle.dump(diccionario_entrada, obj_pkl)
        print ("exito")
##########################

dic_aux = leer_json()
print ("*"*50)
for c in dic_aux.keys():
    print(f"clave :{c}")
    #valor = dic_aux[c]
    if isinstance (dic_aux[c], str):
        print (f"\t{dic_aux[c]} ")
    elif isinstance (dic_aux[c], (list,tuple)):
        for index, v in  enumerate(dic_aux[c]):
            print (f"\t{index}   -  {v} ")
    elif isinstance (dic_aux[c], (dict)):
        for sub_c, sub_v in  dic_aux[c].items():
            print (f"\t{sub_c}   -  {sub_v} ")
dic_aux["acentos"]="áéíóúü"


# ~ with open(f"0000_archivo.json",mode="w", encoding='utf-8' ) as obj_json:
    # ~ json.dump(dic_aux, obj_json, ensure_ascii=False, indent=4 )
    # ~ print ("exito")
grabar_pkl(archivo="instalador_2025",**dic_aux)
# ~ exit()
####################################################################################################
####################################################################################################
#                                                                                                  #
